...
export default function UsersPage() {
  ...
  const [roleSlug, setRoleSlug] = useState<UserRole>('user')
  ...
  const resetForm = () => {
    ...
    setPassword('')
    setRoleSlug('user')
    setLevel(Math.min(40, maxAssignableLevel))
    setCargoId(null)
    setManagerId(null)
    setIsActive(true)
    setEditing(null)
  ...
  const handleRoleSlugChange = (slug: string) => {
    setRoleSlug(slug as UserRole)
    if (cargoId) {
      const c = cargos.find((x) => x.id === cargoId)
      if (c && (c.role !== slug || String(c.level) !== String(level))) {
        setCargoId(null)
      }
    }
  }
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            Hierarquia por nível: só vê e edita utilizadores com nível estritamente inferior ao seu. Roles fixas:
            administrador ou usuário — o poder efetivo vem do nível numérico.
          </p>
        </div>
            ...
            <tbody>
              ...
              {rows.map((u) => (
                <tr key={u.id} className="border-t border-gray-100">
                  ...
                  <td className="whitespace-nowrap px-4 py-3 font-mono text-gray-800">{u.level}</td>
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-4">
              ...
              <div>
                <label htmlFor="uf-cargo" className="block text-xs font-medium text-gray-600">
                  Cargo (opcional — define role e nível)
                </label>
                <select
                  id="uf-cargo"
                  value={cargoId ?? ''}
                  ...
                  className="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm"
                >
                  <option value="">— Definir manualmente abaixo —</option>
                  {cargos.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.name} ({roleLabel(c.role)} · nível {c.level})
                    </option>
                  ))}
                </select>
              </div>
              ...
              {!cargoId && !(modalMode === 'edit' && currentUserId === editing?.id) && (
                <>
                  <div>
                    <label htmlFor="uf-role" className="block text-xs font-medium text-gray-600">
                      Role
                    </label>
                    <select
                      id="uf-role"
                  ...
                  <div>
                    <label htmlFor="uf-level" className="block text-xs font-medium text-gray-600">
                      Nível (máx. {maxAssignableLevel})
                    </label>
                    <input
                      id="uf-level"
                      type="number"
              ...
              {currentUserId === editing?.id && (
                <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-900">
                  Está a editar a sua própria conta. Não pode alterar role, nível ou estado aqui — use só nome,
